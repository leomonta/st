...
void xinit(int cols, int rows) {
	...
	if (file) {
		...
		long     *icon_argb = (long *)(xmalloc(icon_n * sizeof(long)));
		...
		XChangeProperty(xw.dpy, xw.win, xw.netwmicon, XA_CARDINAL, 32, PropModeReplace, (uchar *)icon_argb, icon_n);
		free(icon_argb);
	}
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int charlen, int dmode) {
...
void xdrawline(Line line, int x1, int y1, int x2) {
	...
	/* Draw line in 2 passes: background and foreground. This way wide glyphs won't get truncated */
